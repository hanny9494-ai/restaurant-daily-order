...
export default function RecipeViewPage() {
  ...
  const canEditVersion = Boolean(
    selectedVersion && canEdit && (selectedVersion.status === "DRAFT" || selectedVersion.status === "REJECTED")
  );
  ...
  async function saveRecord() {
    if (!selectedUser || !selectedRecipeId || !selectedVersionId || !record) return;
        ...
        {!detail || !selectedVersion || !viewRecord ? (
          ...
          <>
            <div className="row" style={{ justifyContent: "space-between" }}>
              <span>
                <strong>{viewRecord.meta.dish_name}</strong>（{viewRecord.meta.dish_code}）/{viewRecord.meta.recipe_type}
                {viewRecord.meta.menu_cycle ? `/${viewRecord.meta.menu_cycle}` : ""}
              </span>
              <div className="row">
                ...
                {canEditVersion ? (
                  editMode ? (
                    <>
                      <button className="btn" type="button" onClick={saveRecord}>保存</button>
                    ...
                    <button className="btn" type="button" onClick={() => setEditMode(true)}>进入编辑模式</button>
                  )
                ) : canEdit ? (
                  <button className="btn" type="button" onClick={() => createRevision(true)}>创建修订并编辑</button>
                ) : null}
                <button className="btn secondary" type="button" disabled={!canEditVersion || editMode} onClick={submitForApproval}>提交审批</button>
              </div>
            </div>
